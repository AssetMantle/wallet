...
async function CosmostationWallet() {
    ...
    // If `window.getOfflineSigner` or `window.cosmostation` is null, Keplr extension may be not installed on browser.
    if (!window.getOfflineSigner || !window.cosmostation) {
        throw new Error("install cosmostation extension");
    } else {
        if (window.cosmostation.experimentalSuggestChain) {
            try {
                ...
                await window.cosmostation.experimentalSuggestChain({
    ...
    await window.cosmostation.enable(chainId);
    ...
    //     offlineSigner,
    // );
    localStorage.setItem('cosmostationAddress', accounts[0].address);

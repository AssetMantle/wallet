...
export const mantleCustomChains = {
  mantleMainnetChainConfig: {
    $schema: "../../chain.schema.json",
    chain_name: "assetmantle",
    status: "live",
    network_type: "mainnet",
...
export const mantleCustomAssets = {
  mantleMainnetAssetConfig: {
    $schema: "../../assetlist.schema.json",
    chain_name: "assetmantle",
    assets: [
      {
...
export const getFinalChainsList = () => {
  const chainList = chains.filter(
    (chain) => chain.chain_name !== "assetmantle"
  );

  const mantleCustomChainsArray = Object.keys(mantleCustomChains)?.map?.(
    (mantleCustomChain) => mantleCustomChains?.[mantleCustomChain]
  );

  const finalChains = [...chainList, ...mantleCustomChainsArray];
  return finalChains;
};

export const getFinalAssetsList = () => {
  const assetList = assets.filter(
    (assets) => assets.chain_name !== "assetmantle"
  );

  const mantleCustomAssetsArray = Object.keys(mantleCustomAssets)?.map?.(
    (mantleCustomAsset) => mantleCustomAssets?.[mantleCustomAsset]
  );

  const finalAssets = [...assetList, ...mantleCustomAssetsArray];
  return finalAssets;
};

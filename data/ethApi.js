...
import { toChainDenom, toDenom } from "./queryApi";
...
export const ethConfig = {
  ...
  mainnet: {
    ...
    polygonOpenApi: {
      url: (address) =>
        `https://open-api.polygon.technology/api/v1/burn/address?userAddress=${address}&isPos=true&tokenType=ERC20&page=0&pageSize=20`,
...
const getPosClientConfig = async (address, connector) => {
  console.log("inside getPosClientConfig, connector: ", connector);
  const providerEthereum = await connector?.getProvider?.({
    chainId: mainnet?.id,
  });
  const providerPolygon = await connector?.getProvider?.({
    chainId: polygon?.id,
  });
...
const getPosClientConfig2 = async (address) => {
  const parentProvider = getProvider({
    chainId: 1,
  });

  const childProvider = getProvider({
    chainId: 137,
  });

  const childProviderReady = await childProvider?.ready();
...
export const useCheckpointedBurnTransactions = () => {
  // get the address from connected web3Modal
  const { address } = useAccount();
  const polygonOpenApi = ethConfig?.mainnet?.polygonOpenApi?.url?.(address);
  const polygonOpenApiKey = ethConfig?.mainnet?.polygonOpenApi?.key;
  const polygonMntlToken = ethConfig?.mainnet?.token?.child?.erc20;
  ...
  const fetchBurnTransactions = async (url, address) => {
    ...
    const burnTransactionsArray = data?.result?.filter?.(
      (txnObject) =>
        txnObject?.childToken == polygonMntlToken &&
        txnObject?.transactionStatus == "checkpointed"
    );
...
export const useAllowance = () => {
  ...
  // console.log("connector: ", connector.getProvider());
  ...
  const fetchAllowance = async (url, address, connector) => {
    ...
    try {
      ...
      // get the pos client config object
      const getPosClientConfigObject = await getPosClientConfig(
        address,
        connector
      );

      // initialize POS client
      await posClient.init(getPosClientConfigObject);
...
export const depositMntlToken = async (address, amount, connector) => {
  ...
  let denomAmount = toDenom(amount);
...
export const exitMntlToken = async (address, burnTxHash) => {
  console.log("inside exitMntlToken, burnTxHash: ", burnTxHash);
  let response;
  ...
  try {
    ...
    // get the pos client config object
    const posClientConfigObject = await getPosClientConfig2(address);

    // initialize POS client
    await posClient.init(posClientConfigObject);

    // get the parent ERC20 token
    const parentERC20Token = posClient.erc20(parentERC20TokenAddress, true);

    const transactionResponse = await parentERC20Token.withdrawExit(
      burnTxHash + ""
    );

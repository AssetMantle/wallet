...
export const getTimeDifference = (timestampOne, timestampTwo) => {
  if (BigNumber(timestampOne).isNaN() || BigNumber(timestampTwo).isNaN()) {
    return "";
  }
  const differenceInSecondsBn = BigNumber(timestampOne)
    .minus(BigNumber(timestampTwo))
    .absoluteValue();

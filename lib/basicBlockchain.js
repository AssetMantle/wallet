...
import { defaultChainName, getFinalChainsList } from "../config";
import {
...
export const getPrefixFromChainName = (chainName = defaultChainName) => {
  const chainList = getFinalChainsList();
  const selectedChain = chainList.filter(
    (chain) => chain.chain_name == chainName
  );
  return selectedChain?.[0]?.bech32_prefix || "invalid";
};

export const getSlip44FromChainName = (chainName = defaultChainName) => {
  const chainList = getFinalChainsList();
  const selectedChain = chainList.filter(
    (chain) => chain.chain_name == chainName
  );
  return selectedChain?.[0]?.slip44 || "";
};

export const getChainIdFromChainName = (chainName = defaultChainName) => {
  const chainList = getFinalChainsList();
  const selectedChain = chainList.filter(
    (chain) => chain.chain_name == chainName
  );
  return selectedChain?.[0]?.chain_id;
};
